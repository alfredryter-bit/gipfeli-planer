...
<body>
    ...
    <script>
        ...
        function matchesKonamiKey(event, expected) {
            const key = typeof event.key === 'string' && event.key.length === 1
                ? event.key.toLowerCase()
                : event.key;
            const code = expected.length === 1 ? `Key${expected.toUpperCase()}` : expected;
            return key === expected || event.code === code;
        }

        document.addEventListener('keydown', (e) => {
            if (matchesKonamiKey(e, konamiCode[konamiIndex])) {
                konamiIndex++;
                ...
                konamiIndex = matchesKonamiKey(e, konamiCode[0]) ? 1 : 0;
            }
        });
