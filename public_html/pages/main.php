...
document.addEventListener('keydown', function(e) {
    ...
    const normalizedKey = typeof e.key === 'string' && e.key.length === 1
        ? e.key.toLowerCase()
        : e.key;
    const expectedCode = expectedKey.length === 1 ? 'Key' + expectedKey.toUpperCase() : expectedKey;

    // Use both key and code so Caps Lock and keyboard layouts do not break the trigger.
    if (normalizedKey === expectedKey || e.code === expectedCode) {
        konamiIndex++;
        ...
        konamiIndex = normalizedKey === 'ArrowUp' || e.code === 'ArrowUp' ? 1 : 0;
    }
});
...
function activateEasterEgg() {
    ...
    function animateGipfeli(element, startY, startRotation, endRotation, duration, delay) {
        const fromTransform = 'translate3d(0, ' + startY + 'px, 0) rotate(' + startRotation + 'deg)';
        const toTransform = 'translate3d(0, 110vh, 0) rotate(' + endRotation + 'deg)';

        if (typeof element.animate === 'function') {
            element.animate(
                [{ transform: fromTransform }, { transform: toTransform }],
                { duration: duration * 1000, delay: delay * 1000, easing: 'linear', fill: 'both' }
            );
            return;
        }
    ...
    for (let i = 0; i < gipfeliCount; i++) {
        ...
        gipfeli.style.pointerEvents = 'none';
        ...
        animateGipfeli(gipfeli, startY, startRotation, endRotation, duration, delay);
