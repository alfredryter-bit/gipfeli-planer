...
<head>
    ...
    <title><?php echo htmlspecialchars(t('auth.reset_confirm')); ?> - <?php echo htmlspecialchars($appName); ?></title>
...
<body>
    ...
    <div class="reset-container">
        <div class="logo-container">
            ...
            <p class="description"><?php echo htmlspecialchars(t('auth.reset_confirm')); ?></p>
        </div>
        ...
        <?php if (empty($token)): ?>
            <div class="alert alert-danger">
                <?php echo htmlspecialchars(t('auth.reset_invalid_token')); ?>
            </div>
            <div class="links">
                <a href="?page=reset-password"><?php echo htmlspecialchars(t('auth.reset')); ?></a>
                <br><br>
        ...
        <?php else: ?>
            <form id="confirm-form">
                ...
                <div class="form-group">
                    <label for="password"><?php echo htmlspecialchars(t('auth.password_new')); ?></label>
                ...
                <div class="form-group">
                    <label for="confirm-password"><?php echo htmlspecialchars(t('auth.password_confirm')); ?></label>
                    <input type="password" id="confirm-password" name="confirm-password" required minlength="10" maxlength="128">
                </div>
                
                <button type="submit" id="submit-btn"><?php echo htmlspecialchars(t('auth.update_password')); ?></button>
            </form>
    ...
    <script>
        const i18n = {
            requirements: <?php echo json_encode(t('auth.error.password_requirements')); ?>,
            mismatch: <?php echo json_encode(t('auth.error.password_mismatch')); ?>,
            updating: <?php echo json_encode(t('auth.updating')); ?>,
            updateAction: <?php echo json_encode(t('auth.update_password')); ?>,
            updateSuccess: <?php echo json_encode(t('auth.password_updated_default')); ?>,
            updateError: <?php echo json_encode(t('auth.error.reset_confirm_failed')); ?>,
            unexpectedError: <?php echo json_encode(t('auth.error.unexpected')); ?>
        };
        ...
        confirmForm.addEventListener('submit', async function(e) {
            ...
            const policy = checkPasswordPolicy(password);
            if (!policy.valid) {
                showMessage(i18n.requirements, 'danger');
                return;
            }
            
            if (password !== confirmPassword) {
                showMessage(i18n.mismatch, 'danger');
                return;
            }
            
            // Button deaktivieren, um mehrfaches Absenden zu verhindern
            submitBtn.disabled = true;
            submitBtn.textContent = i18n.updating;
            ...
            try {
                ...
                if (data.success) {
                    ...
                    showMessage(data.message || i18n.updateSuccess, 'success');
                    ...
                    // Fehler anzeigen
                    showMessage(data.error || i18n.updateError, 'danger');
                    submitBtn.disabled = false;
                    submitBtn.textContent = i18n.updateAction;
                }
            } catch (error) {
                showMessage(i18n.unexpectedError, 'danger');
